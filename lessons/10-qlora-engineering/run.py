#!/usr/bin/env python3
"""Lesson 10: QLoRA and training engineering planner.

This lesson intentionally does not import bitsandbytes or launch CUDA training.
...
LESSON_ID = "10-qlora-engineering"
LESSON_TITLE = "Lesson 10 · QLoRA / Training Engineering"
...
def summarize_boundaries(machine: dict[str, Any]) -> dict[str, str]:
    ...
    return {
        ...
        "mac_mps": (
            "MPS is available, but this lesson does not present bitsandbytes 4-bit QLoRA as native on Mac."
            if has_mps
...
def main() -> None:
    ...
    parser.add_argument("--config", default="lessons/10-qlora-engineering/data/planning_config.json")
    parser.add_argument("--model-name", default="auto")
    parser.add_argument("--report", default="lessons/10-qlora-engineering/report.md")
    parser.add_argument("--index", default="lessons/10-qlora-engineering/index.html")
    parser.add_argument("--output-dir", default="lessons/10-qlora-engineering/outputs")
    parser.add_argument("--trace", default="lessons/10-qlora-engineering/outputs/trace.json")
    ...
    trace.event(
        ...
        "setup",
        "读取 Lesson 10 的训练工程配置。配置只包含规划参数，不包含 CUDA 训练入口。",
        inputs={"config": args.config},
        outputs={"profiles": [profile["name"] for profile in config["profiles"]], "quick": args.quick},
    )
    ...
    trace.event(
        ...
        "setup",
        "用课程已有 hf_model_policy helper 检测本机并解析 --model-name auto。",
    ...
    trace.event(
        ...
        "metadata",
        "尝试读取 Hugging Face config/tokenizer 元数据；这一步不下载模型权重，失败也不影响本课执行。",
    ...
    trace.event(
        "write lesson artifacts",
        "artifact",
        "生成本课报告和静态学习页，所有运行产物保存在 Lesson 10 outputs。",
        outputs={"report": report_path, "index": index_path, **paths},
    )
    trace.finish(
        "QLoRA planning lesson completed without requiring bitsandbytes, CUDA, or model-weight downloads.",
