#!/usr/bin/env python3
"""Shared helpers for local fine-tuning runs."""
...
def ensure_local_auto_tokenizer(
    data_path: str | Path,
    tokenizer_dir: str | Path = "training/runtime/local-sft-tokenizer",
    vocab_size: int = 256,
):
    """Train a tiny local tokenizer and load it through AutoTokenizer.

    This keeps the run offline while still using the real Hugging Face fast
