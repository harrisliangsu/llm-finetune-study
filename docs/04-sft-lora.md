...
严格说，当前课程有 SFT 的数据、labels 和训练目标，但没有独立的一课叫 SFT Baseline。Lesson 05 已经进入 LoRA，但用的是本地 tiny model，不下载真实 LLM。Lesson 06 使用 `Qwen/Qwen2.5-0.5B-Instruct` 跑通真实 PEFT LoRA。继续往后学习前，需要补一个独立 SFT 课，明确 SFT 到底如何改变模型回答，再看 adapter 评估和 DPO。
...
完成 Lesson 06 后，建议继续阅读 [07. 开发者训练方法指南](07-training-methods-guide.md)。下一课应先补 SFT Baseline，然后才是 adapter 评估、DPO 和 reward/RLHF 概念实验。
