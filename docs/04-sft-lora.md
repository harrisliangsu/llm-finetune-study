...
严格说，当前课程有 SFT 的数据、labels 和训练目标，但没有独立的一课叫 SFT Baseline。Lesson 05 已经进入 LoRA，但用的是本地 tiny model，不下载真实 LLM。Lesson 06 使用 `Qwen/Qwen2.5-0.5B-Instruct` 跑通真实 PEFT LoRA，并且已经比较了 base、训练后 LoRA、重新加载 adapter 的固定 prompt 输出。继续往后学习前，需要补一个独立 SFT 课，明确 SFT 到底如何改变模型回答，再看 DPO。

Lesson 06 的训练前后效果不明显，主要不是 PEFT 失效，而是课程展示目标选得不够好：样本只有 5 条，任务是模型本来就会的通用概念解释，UI 也没有把 before/after 放在第一层。独立 SFT Baseline 应改用结构化输出任务，例如客服工单路由到严格 JSON，并使用 40-80 条同分布样本。
...
完成 Lesson 06 后，建议继续阅读 [07. 开发者训练方法指南](07-training-methods-guide.md)。下一课应先补 SFT Baseline，然后才是 DPO 和 reward/RLHF 概念实验。
